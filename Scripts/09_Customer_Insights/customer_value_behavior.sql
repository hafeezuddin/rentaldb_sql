...
--CTE for each customers fav category
     fav_cat AS (SELECT t1.customer_id, t1.name
                 FROM (SELECT ci.customer_id,
                              ...
                              row_number()
                              OVER (PARTITION BY ci.customer_id ORDER BY count(distinct r.rental_id) DESC, ct.name ASC) AS ctrank
                       ...
                       WHERE r.rental_date >= '2005-01-01'
                         AND r.rental_date <= '2005-12-31' --Considering 2005 rentals data
                       group by ci.customer_id, ct.name) t1
                 WHERE t1.ctrank = 1),
--CTE for primary store customer uses.
     primary_store AS (SELECT t2.customer_id, t2.staff_id, t2.store_id
                       FROM (SELECT ci.customer_id,
                                    st.staff_id,
                                    st.store_id,
                                    COUNT(distinct r.rental_id),
                                    row_number()
                                    OVER (PARTITION BY ci.customer_id order by COUNT(DISTINCT r.rental_id) DESC, st.store_id ASC) favrank
                             FROM customer_insights ci
                                      INNER JOIN rental r ON ci.customer_id = r.customer_id
...
SELECT ci.customer_id,
       ...
       ci.risk_status,
       ci.last_rental_date,
       ci.days_since_last_rental,
       ci.life_time_payments,
       life_time_value_percent_rank * 100 AS spent_rank,
       ci.rental_frequency,
       ci.total_rentals,
       fc.name,
       ps.staff_id,
