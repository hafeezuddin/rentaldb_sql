...
--Base CTE to retrieve rental information of each customer
     base_cte AS (SELECT DISTINCT r.customer_id, TO_CHAR(DATE_TRUNC('MONTH', r.rental_date), 'YYYY-MM') AS ym
                  FROM rental r
                           JOIN payment p
...
SELECT bc.customer_id, COUNT(DISTINCT bc.ym) total_unique_rental_months
FROM base_cte bc
