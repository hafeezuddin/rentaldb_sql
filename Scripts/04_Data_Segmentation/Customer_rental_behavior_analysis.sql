...
--CTE to calculate basic metrics - total rentals & percentile rank, total amount spent & corresponding percentile rank
WITH customer_metrics AS (
    SELECT c.customer_id,
        COUNT(DISTINCT r.rental_id) AS total_rentals,
        PERCENT_RANK() OVER (ORDER BY SUM(p.amount)) AS p_total_spent,
        SUM(p.amount) AS total_spent,
        PERCENT_RANK() OVER (ORDER BY COUNT(DISTINCT r.rental_id)) AS p_total_rentals
    FROM customer c
...
derived_metrics AS (
    ...
    FROM customer_metrics cm
        ...
        WHERE r2.return_date IS NOT NULL AND (r2.rental_date >= '2005-01-01' AND r2.rental_date <= '2005-12-31')
    ...
    SELECT dm.customer_id,
        ...
        CASE
            WHEN LAG(dm.date_of_rental) OVER (PARTITION BY dm.customer_id ORDER BY dm.date_of_rental DESC) IS NULL
                THEN 0
            ELSE LAG(dm.date_of_rental) OVER (PARTITION BY dm.customer_id ORDER BY dm.date_of_rental DESC) - dm.date_of_rental
        END AS diff
...
most_rented_category AS (
    SELECT x.customer_id, x.name
    FROM (
            SELECT cm.customer_id, cat.name,
            ...
            FROM customer_metrics cm
                ...
                WHERE r3.return_date IS NOT NULL AND (r3.rental_date >= '2005-01-01' AND r3.rental_date <= '2005-12-31')
...
active_status AS (
    ...
    FROM customer_metrics cm
        ...
        WHERE r4.return_date IS NOT NULL AND (r4.rental_date >= '2005-01-01' AND r4.rental_date <= '2005-12-31')
    GROUP BY 1
    ORDER BY 1
),
averages_counts AS (
