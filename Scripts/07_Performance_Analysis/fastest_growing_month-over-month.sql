...
pmr_cal AS (
SELECT
  cs.category_id, cs.name, cs.month, cs.total_revenue,
  LAG(cs.total_revenue) OVER (PARTITION BY cs.category_id ORDER BY cs.month) AS pmr
  FROM cat_stats cs
),
--Change in percentage when compared to previous month
per_change_cal AS (
  SELECT
    pmr_cal.category_id, pmr_cal.name, pmr_cal.month, pmr_cal.total_revenue, pmr_cal.pmr,
    CASE
...
ranking_months AS (
  Select
  pcl.category_id, pcl.name, pcl.month, pcl.total_revenue, pcl.pmr,
